...
enum MENZA {MAIN_PAGE, LOGIN_PAGE, KONTA_PAGE, /*ORDERS_PAGE,*/ ERROR}
...
class _WebViewMenzaPageState extends State<WebViewMenzaPage> {
  ...
  MENZA pageState = MENZA.MAIN_PAGE;
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageFinished: (url) async {
            ...
            if (url.contains('https://webiskam.mendelu.cz/Konta') && (pageState == MENZA.LOGIN_PAGE || pageState == MENZA.MAIN_PAGE)) { // KONTA PAGE -> DONE
              print("RESULT5");
              pageState = MENZA.KONTA_PAGE;
              controller.runJavaScriptReturningResult("setTimeout(function() {SHOWWEBVIEWtoFlutter.postMessage('SHOW');}, 330);");
            } else if (url.contains('https://webiskam.mendelu.cz/ObjednavkyStravovani') && (pageState == MENZA.LOGIN_PAGE || pageState == MENZA.MAIN_PAGE)) { // LOGGED IN -> show konta page
              controller.loadRequest(Uri.parse('https://webiskam.mendelu.cz/Konta'));
              print("RESULT4");
            } else if (url.contains('https://webiskam.mendelu.cz/Home/Index?ReturnUrl=%2FObjednavkyStravovani') && pageState == MENZA.MAIN_PAGE) { // NOT LOGGED IN -> show login form
              if ((await controller.runJavaScriptReturningResult(
                ...
                print(result);
                if (result == "false") {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("ERROR when connecting to https://webiskam.mendelu.cz, please try again.")));
                  Navigator.of(context).pop();
                  pageState = MENZA.ERROR;
                } else {
                  pageState = MENZA.LOGIN_PAGE;
                }
              }
            }

            if (pageState == MENZA.LOGIN_PAGE) { // LOGGING IN...
              if ((url.contains('https://alibaba.mendelu.cz/idp/profile/SAML2/Redirect/SSO;jsessionid=') || url.contains('https://alibaba.mendelu.cz/idp/profile/SAML2/Redirect/SSO?execution=')) && (await controller.runJavaScriptReturningResult(
                ...
                if (dataUsername.isEmpty || dataPassword.isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("ERROR you are not logged in to MENDELU!")));
                  Navigator.of(context).pop();
                  pageState = MENZA.ERROR;
                }

                final result = (await controller
                    .runJavaScriptReturningResult( //'document.getElementById("username") != null'
                ...
                print(result);
                if (result == "false") {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("ERROR when logging in to https://webiskam.mendelu.cz, please try again.")));
                  Navigator.of(context).pop();
                  pageState = MENZA.ERROR;
                }
